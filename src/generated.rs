...
pub const XDR_FILES_SHA256: [(&str, &str); 13] = [
    ...
    (
        "xdr/Stellar-contract-spec.x",
        "deea6f35d5454fcf662b1006d21594f0bb79884c54ed60601eaf3082046e0009",
    ),
    (
        "xdr/Stellar-contract.x",
...
/// type generated by this crate has, on its borrowing `Ref` form (or on the
/// type itself when it owns no heap data), an inherent `const fn
/// const_write_xdr` that serializes itself through a `ConstWriter`, plus
/// `const_to_xdr` and `const_xdr_len` built on top of it.
///
...
/// first (as `const_xdr_len` does), then serializing again into a buffer of
/// that size (as `const_to_xdr` does).
#[cfg(feature = "const")]
