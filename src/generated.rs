...
pub const XDR_FILES_SHA256: [(&str, &str); 13] = [
    (
        "xdr/Stellar-SCP.x",
        "41ff1f6a3de9fcb07372953e73a9e49071e278081e411a62e69683ad3591a218",
    ),
    (
        "xdr/Stellar-contract-config-setting.x",
    ...
    (
        "xdr/Stellar-contract-env-meta.x",
        "e1f0595afd6249879ea4898f0ccb2293025c21db3a6590178ba129bc1e99d8c9",
    ),
    (
        "xdr/Stellar-contract-meta.x",
    ...
    (
        "xdr/Stellar-exporter.x",
        "d36ec815cceb534e3f565448aa79ba8dd7f6be6485de050dfb62613eb05b11a4",
    ),
    (
        "xdr/Stellar-internal.x",
        "81f9804d6a5f88afdb62e8d00506c348e45c7b70012d781d14ef22ddc7414833",
    ),
    (
        "xdr/Stellar-ledger-entries.x",
...
/// `ConstError` enumerates the errors that can occur while serializing with a
/// [`ConstWriter`]. Unlike [`Error`], it is `Copy` and free of any destructor,
/// which is required for it to be used within const functions.
#[cfg(feature = "std")]
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum ConstError {
    /// The depth limit was exceeded. Maps to [`Error::DepthLimitExceeded`].
    DepthLimitExceeded,
    /// The length limit was exceeded. Maps to [`Error::LengthLimitExceeded`].
    LengthLimitExceeded,
    /// A length exceeded the maximum representable value. Maps to
    /// [`Error::LengthExceedsMax`].
    LengthExceedsMax,
}
...
#[cfg(feature = "std")]
impl ConstError {
    /// Converts this error into the crate's [`Error`] type.
    #[must_use]
    pub const fn to_error(self) -> Error {
        match self {
            ConstError::DepthLimitExceeded => Error::DepthLimitExceeded,
            ConstError::LengthLimitExceeded => Error::LengthLimitExceeded,
            ConstError::LengthExceedsMax => Error::LengthExceedsMax,
        }
    }
}
...
/// operations, while enforcing the same depth and length limits as
/// [`Limited`].
///
/// It is the const-evaluable counterpart to [`WriteXdr::write_xdr`]. Each XDR
/// type generated by this crate has an inherent `const fn const_write_xdr` that
/// serializes itself through a `ConstWriter`, and the type's `to_xdr` is
/// implemented in terms of it.
///
/// Errors are recorded on the writer rather than returned, because a
/// `Result` carrying [`Error`] cannot be dropped inside a const function.
/// After the first error every subsequent operation is a no-op, mirroring the
/// short-circuiting of the `?` operator in [`WriteXdr::write_xdr`]. Inspect
/// [`ConstWriter::error`] once serialization is complete.
///
/// Bytes are only stored while the current position is within the buffer;
/// positions beyond the buffer are counted but not written. This allows the
/// exact encoded length to be measured by serializing into an empty buffer
/// first, then serializing again into a buffer of that size (as
/// [`WriteXdr::to_xdr`] does).
#[cfg(feature = "std")]
pub struct ConstWriter<'a> {
    ...
    depth: u32,
    len: usize,
    error: Option<ConstError>,
}

#[cfg(feature = "std")]
impl<'a> ConstWriter<'a> {
    /// Constructs a new `ConstWriter` that serializes into `buf`, enforcing
    /// `limits`.
    #[must_use]
    pub const fn new(buf: &'a mut [u8], limits: &Limits) -> Self {
        ConstWriter {
            buf,
            pos: 0,
            depth: limits.depth,
    ...
    /// encoded length once serialization completes successfully (even if it
    /// exceeded the buffer).
    #[must_use]
    ...
    /// Returns the first error encountered while serializing, if any.
    #[must_use]
    pub const fn error(&self) -> Option<ConstError> {
        self.error
    }

    /// Returns true while no error has been encountered.
    const fn is_ok(&self) -> bool {
        self.error.is_none()
    }

    /// Records the first error encountered. Later errors are ignored so that
    /// the earliest failure is preserved, matching `?` short-circuiting.
    const fn fail(&mut self, e: ConstError) {
        if self.error.is_none() {
            self.error = Some(e);
        }
    }

    /// Consumes a single depth on entering a nested value, mirroring the depth
    /// accounting of [`Limited::with_limited_depth`]. Every call must be paired
    /// with a later [`ConstWriter::leave_depth`].
    pub const fn enter_depth(&mut self) {
        if !self.is_ok() {
            return;
        }
        match self.depth.checked_sub(1) {
            Some(depth) => self.depth = depth,
            None => self.fail(ConstError::DepthLimitExceeded),
        }
    }

    /// Restores a single depth on leaving a nested value, mirroring the depth
    /// accounting of [`Limited::with_limited_depth`]. Pairs with
    /// [`ConstWriter::enter_depth`].
    pub const fn leave_depth(&mut self) {
        if !self.is_ok() {
            return;
        }
    ...
    /// Consumes the given length from the remaining length limit, mirroring
    /// [`Limited::consume_len`].
    const fn consume_len(&mut self, len: usize) {
        if !self.is_ok() {
            return;
        }
        match self.len.checked_sub(len) {
            Some(len) => self.len = len,
            None => self.fail(ConstError::LengthLimitExceeded),
        }
    }

    /// Writes bytes into the buffer, advancing the position. Bytes beyond the
    ...
    /// Writes `n` zero bytes into the buffer, advancing the position. Bytes
    /// beyond the end of the buffer are counted but not stored.
    const fn write_zeros(&mut self, n: usize) {
        if !self.is_ok() {
            return;
        }
        let mut i = 0;
    ...
    /// Serializes an `f32`. Floating point serialization is unimplemented,
    /// mirroring `<f32 as WriteXdr>::write_xdr`.
    pub const fn write_f32(&mut self, _v: f32) {
        todo!()
    }

    /// Serializes an `f64`. Floating point serialization is unimplemented,
    /// mirroring `<f64 as WriteXdr>::write_xdr`.
    pub const fn write_f64(&mut self, _v: f64) {
        todo!()
    }
    ...
    /// Serializes a `u32` length prefix from a `usize`, failing with
    /// [`ConstError::LengthExceedsMax`] if it does not fit, mirroring the
    /// `len.try_into()` and `len.write_xdr(w)` of the variable-length
    /// `WriteXdr` implementations.
    #[allow(clippy::cast_possible_truncation)]
    pub const fn write_length_prefix(&mut self, len: usize) {
        if !self.is_ok() {
            return;
        }
        if len > u32::MAX as usize {
            self.fail(ConstError::LengthExceedsMax);
            return;
        }
        self.write_u32(len as u32);
    }

    /// Serializes a length-prefixed byte sequence with trailing padding,
    /// mirroring `<VecM<u8> as WriteXdr>::write_xdr`, `<BytesM as
    /// WriteXdr>::write_xdr`, and `<StringM as WriteXdr>::write_xdr`.
    pub const fn write_len_prefixed(&mut self, data: &[u8]) {
        if !self.is_ok() {
            return;
        }
        self.enter_depth();
...
/// Serializes `value` to a `Vec<u8>` using its const serializer.
///
/// This measures the encoded length by serializing into an empty buffer, then
/// allocates a buffer of exactly that size and serializes into it. It is used
/// to implement the `to_xdr` method of every generated type in terms of its
/// `const_write_xdr`.
#[cfg(feature = "std")]
pub(crate) fn to_xdr_via_const<T>(
    value: &T,
    limits: &Limits,
    encode: fn(&T, &mut ConstWriter<'_>),
) -> Result<Vec<u8>, Error> {
    let mut empty: [u8; 0] = [];
    let mut measure = ConstWriter::new(&mut empty, limits);
    encode(value, &mut measure);
    if let Some(e) = measure.error() {
        return Err(e.to_error());
    }
    let len = measure.position();
    let mut buf = vec![0u8; len];
    let mut writer = ConstWriter::new(&mut buf, limits);
