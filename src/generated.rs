...
/// type generated by this crate has, on its borrowing `View` form (or on the
...
impl<'a> ConstWriter<'a> {
    ...
    /// Consumes the length budget for `data`, then writes it into the buffer,
    /// advancing the position. Bytes beyond the end of the buffer are counted
    /// but not stored. Mirrors the streaming path, where every `consume_len` is
    /// paired with a write of exactly that many bytes.
    const fn write_bytes(&mut self, data: &[u8]) {
        self.consume_len(data.len());
        if !self.is_ok() {
            return;
        }
        let mut i = 0;
    ...
    /// Consumes the length budget for `n` bytes, then writes that many zero
    /// bytes into the buffer, advancing the position. Bytes beyond the end of
    /// the buffer are counted but not stored.
