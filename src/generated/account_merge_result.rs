...
impl AccountMergeResult {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_account_merge_result(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`AccountMergeResult`], mirroring `<AccountMergeResult as WriteXdr>::write_xdr`.
    pub const fn write_type_account_merge_result(&mut self, v: &AccountMergeResult) {
        let d = v.discriminant();
        self.write_type_account_merge_result_code(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            AccountMergeResult::Success(value) => {
                self.write_i64(*value);
            }
            AccountMergeResult::Malformed => {}
            AccountMergeResult::NoAccount => {}
            AccountMergeResult::ImmutableSet => {}
            AccountMergeResult::HasSubEntries => {}
            AccountMergeResult::SeqnumTooFar => {}
            AccountMergeResult::DestFull => {}
