...
impl AllowTrustOp {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_allow_trust_op(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`AllowTrustOp`], mirroring `<AllowTrustOp as WriteXdr>::write_xdr`.
    pub const fn write_type_allow_trust_op(&mut self, v: &AllowTrustOp) {
        self.write_type_account_id(&v.trustor);
        self.write_type_asset_code(&v.asset);
        self.write_u32(v.authorize);
    }
}
