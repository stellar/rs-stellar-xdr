...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`AssetCode`], mirroring `<AssetCode as WriteXdr>::write_xdr`.
    pub const fn write_type_asset_code(&mut self, v: &AssetCode) {
        let d = v.discriminant();
        self.write_type_asset_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            AssetCode::CreditAlphanum4(value) => {
                self.write_type_asset_code4(value);
            }
            AssetCode::CreditAlphanum12(value) => {
                self.write_type_asset_code12(value);
            }
        }
    }
}
