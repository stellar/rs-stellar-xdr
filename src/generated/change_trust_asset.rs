...
impl ChangeTrustAsset {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_change_trust_asset(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ChangeTrustAsset`], mirroring `<ChangeTrustAsset as WriteXdr>::write_xdr`.
    pub const fn write_type_change_trust_asset(&mut self, v: &ChangeTrustAsset) {
        let d = v.discriminant();
        self.write_type_asset_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            ChangeTrustAsset::Native => {}
            ChangeTrustAsset::CreditAlphanum4(value) => {
                self.write_type_alpha_num4(value);
            }
            ChangeTrustAsset::CreditAlphanum12(value) => {
                self.write_type_alpha_num12(value);
            }
            ChangeTrustAsset::PoolShare(value) => {
                self.write_type_liquidity_pool_parameters(value);
            }
        }
    }
}
