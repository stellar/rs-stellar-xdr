...
impl ConfigUpgradeSetKey {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_config_upgrade_set_key(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ConfigUpgradeSetKey`], mirroring `<ConfigUpgradeSetKey as WriteXdr>::write_xdr`.
    pub const fn write_type_config_upgrade_set_key(&mut self, v: &ConfigUpgradeSetKey) {
        self.write_type_contract_id(&v.contract_id);
        self.write_type_hash(&v.content_hash);
    }
}
