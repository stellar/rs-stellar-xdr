...
impl InflationPayout {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_inflation_payout(self);
        w.len()
    }
...
impl ConstWriter<'_> {
    /// Serializes a [`InflationPayout`], mirroring `<InflationPayout as WriteXdr>::write_xdr`.
    pub const fn write_type_inflation_payout(&mut self, v: &InflationPayout) {
    ...
    pub const fn write_type_vec_inflation_payout<const MAX: u32>(
        ...
        let mut i = 0usize;
        while i < len {
            self.write_type_inflation_payout(&s[i]);
            i += 1;
        }
    }
}
