...
/// ParallelTxExecutionStage is a borrowing equivalent of [`ParallelTxExecutionStage`](super::super::ParallelTxExecutionStage)
...
impl ConstWriter<'_> {
    ...
    pub const fn write_type_parallel_tx_execution_stage(&mut self, v: &ParallelTxExecutionStage) {
    ...
    pub const fn write_type_vec_parallel_tx_execution_stage<const MAX: u32>(
        &mut self,
        v: &VecM<ParallelTxExecutionStage, MAX>,
    ) {
        let s = v.as_slice();
        let len = s.len();
        self.write_len(len);
