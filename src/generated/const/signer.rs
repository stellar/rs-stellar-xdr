...
/// Signer is a borrowing equivalent of [`Signer`](super::super::Signer)
...
impl ConstWriter<'_> {
    ...
    pub const fn write_type_signer(&mut self, v: &Signer) {
    ...
    pub const fn write_type_option_signer(&mut self, v: &Option<Signer>) {
        match v {
            Some(v) => {
                self.write_u32(1);
    ...
    pub const fn write_type_vec_signer<const MAX: u32>(&mut self, v: &VecM<Signer, MAX>) {
        let s = v.as_slice();
        let len = s.len();
        self.write_len(len);
        let mut i = 0usize;
