...
/// SorobanAuthorizationEntry is a borrowing equivalent of [`SorobanAuthorizationEntry`](super::super::SorobanAuthorizationEntry)
...
impl SorobanAuthorizationEntry {
...
impl ConstWriter<'_> {
    ...
    pub const fn write_type_soroban_authorization_entry(&mut self, v: &SorobanAuthorizationEntry) {
    ...
    pub const fn write_type_vec_soroban_authorization_entry<const MAX: u32>(
        &mut self,
        v: &VecM<SorobanAuthorizationEntry, MAX>,
    ) {
        let s = v.as_slice();
        let len = s.len();
        self.write_len(len);
