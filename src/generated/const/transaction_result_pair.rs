...
/// TransactionResultPair is a borrowing equivalent of [`TransactionResultPair`](super::super::TransactionResultPair)
...
impl ConstWriter<'_> {
    ...
    pub const fn write_type_transaction_result_pair(&mut self, v: &TransactionResultPair) {
    ...
    pub const fn write_type_vec_transaction_result_pair<const MAX: u32>(
        &mut self,
        v: &VecM<TransactionResultPair, MAX>,
    ) {
        let s = v.as_slice();
        let len = s.len();
        self.write_len(len);
