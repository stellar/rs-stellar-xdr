...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`DontHave`], mirroring `<DontHave as WriteXdr>::write_xdr`.
    pub const fn write_type_dont_have(&mut self, v: &DontHave) {
        self.write_type_message_type(&v.type_);
        self.write_type_uint256(&v.req_hash);
    }
}
