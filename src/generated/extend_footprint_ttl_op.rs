...
impl ExtendFootprintTtlOp {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_extend_footprint_ttl_op(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ExtendFootprintTtlOp`], mirroring `<ExtendFootprintTtlOp as WriteXdr>::write_xdr`.
    pub const fn write_type_extend_footprint_ttl_op(&mut self, v: &ExtendFootprintTtlOp) {
        self.write_type_extension_point(&v.ext);
        self.write_u32(v.extend_to);
    }
}
