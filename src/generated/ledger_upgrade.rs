...
impl LedgerUpgrade {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_ledger_upgrade(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`LedgerUpgrade`], mirroring `<LedgerUpgrade as WriteXdr>::write_xdr`.
    pub const fn write_type_ledger_upgrade(&mut self, v: &LedgerUpgrade) {
        let d = v.discriminant();
        self.write_type_ledger_upgrade_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            LedgerUpgrade::Version(value) => {
                self.write_u32(*value);
            }
            LedgerUpgrade::BaseFee(value) => {
                self.write_u32(*value);
            }
            LedgerUpgrade::MaxTxSetSize(value) => {
                self.write_u32(*value);
            }
            LedgerUpgrade::BaseReserve(value) => {
                self.write_u32(*value);
            }
            LedgerUpgrade::Flags(value) => {
                self.write_u32(*value);
            }
            LedgerUpgrade::Config(value) => {
                self.write_type_config_upgrade_set_key(value);
            }
            LedgerUpgrade::MaxSorobanTxSetSize(value) => {
                self.write_u32(*value);
            }
        }
    }
