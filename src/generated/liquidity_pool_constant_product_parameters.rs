...
impl LiquidityPoolConstantProductParameters {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_liquidity_pool_constant_product_parameters(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`LiquidityPoolConstantProductParameters`], mirroring `<LiquidityPoolConstantProductParameters as WriteXdr>::write_xdr`.
    pub const fn write_type_liquidity_pool_constant_product_parameters(
        &mut self,
        v: &LiquidityPoolConstantProductParameters,
    ) {
        self.write_type_asset(&v.asset_a);
        self.write_type_asset(&v.asset_b);
        self.write_i32(v.fee);
    }
}
