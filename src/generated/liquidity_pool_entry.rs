...
impl LiquidityPoolEntry {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_liquidity_pool_entry(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`LiquidityPoolEntry`], mirroring `<LiquidityPoolEntry as WriteXdr>::write_xdr`.
    pub const fn write_type_liquidity_pool_entry(&mut self, v: &LiquidityPoolEntry) {
        self.write_type_pool_id(&v.liquidity_pool_id);
        self.write_type_liquidity_pool_entry_body(&v.body);
    }
}
