...
#[cfg(feature = "alloc")]
impl IntoOwned for ManageBuyOfferOp {
    type Owned = ManageBuyOfferOp;
    fn into_owned(self) -> ManageBuyOfferOp {
        self
    }
}
...
impl ManageBuyOfferOp {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_manage_buy_offer_op(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ManageBuyOfferOp`], mirroring `<ManageBuyOfferOp as WriteXdr>::write_xdr`.
    pub const fn write_type_manage_buy_offer_op(&mut self, v: &ManageBuyOfferOp) {
        self.write_type_asset(&v.selling);
        self.write_type_asset(&v.buying);
        self.write_i64(v.buy_amount);
        self.write_type_price(&v.price);
