...
impl ManageDataResult {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_manage_data_result(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ManageDataResult`], mirroring `<ManageDataResult as WriteXdr>::write_xdr`.
    pub const fn write_type_manage_data_result(&mut self, v: &ManageDataResult) {
        let d = v.discriminant();
        self.write_type_manage_data_result_code(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            ManageDataResult::Success => {}
            ManageDataResult::NotSupportedYet => {}
            ManageDataResult::NameNotFound => {}
            ManageDataResult::LowReserve => {}
            ManageDataResult::InvalidName => {}
        }
    }
}
