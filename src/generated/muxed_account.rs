...
impl MuxedAccount {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_muxed_account(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`MuxedAccount`], mirroring `<MuxedAccount as WriteXdr>::write_xdr`.
    pub const fn write_type_muxed_account(&mut self, v: &MuxedAccount) {
        let d = v.discriminant();
        self.write_type_crypto_key_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            MuxedAccount::Ed25519(value) => {
                self.write_type_uint256(value);
            }
            MuxedAccount::MuxedEd25519(value) => {
                self.write_type_muxed_account_med25519(value);
            }
        }
    }

    /// Serializes an optional [`MuxedAccount`], mirroring `<Option<MuxedAccount> as WriteXdr>::write_xdr`.
    pub const fn write_type_option_muxed_account(&mut self, v: &Option<MuxedAccount>) {
        match v {
            Some(v) => {
                self.write_u32(1);
                self.write_type_muxed_account(v);
            }
            None => {
                self.write_u32(0);
            }
