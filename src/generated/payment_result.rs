...
impl PaymentResult {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_payment_result(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`PaymentResult`], mirroring `<PaymentResult as WriteXdr>::write_xdr`.
    pub const fn write_type_payment_result(&mut self, v: &PaymentResult) {
        let d = v.discriminant();
        self.write_type_payment_result_code(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            PaymentResult::Success => {}
            PaymentResult::Malformed => {}
