...
impl PeerAddressIp {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_peer_address_ip(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`PeerAddressIp`], mirroring `<PeerAddressIp as WriteXdr>::write_xdr`.
    pub const fn write_type_peer_address_ip(&mut self, v: &PeerAddressIp) {
        let d = v.discriminant();
        self.write_type_ip_addr_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
