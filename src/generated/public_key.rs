...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`PublicKey`], mirroring `<PublicKey as WriteXdr>::write_xdr`.
    pub const fn write_type_public_key(&mut self, v: &PublicKey) {
        let d = v.discriminant();
        self.write_type_public_key_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
