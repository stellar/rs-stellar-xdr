...
impl ScEnvMetaEntry {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_sc_env_meta_entry(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ScEnvMetaEntry`], mirroring `<ScEnvMetaEntry as WriteXdr>::write_xdr`.
    pub const fn write_type_sc_env_meta_entry(&mut self, v: &ScEnvMetaEntry) {
        let d = v.discriminant();
        self.write_type_sc_env_meta_kind(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            ScEnvMetaEntry::ScEnvMetaKindInterfaceVersion(value) => {
                self.write_type_sc_env_meta_entry_interface_version(value);
            }
        }
    }
}
