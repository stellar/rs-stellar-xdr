...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`ScError`], mirroring `<ScError as WriteXdr>::write_xdr`.
    pub const fn write_type_sc_error(&mut self, v: &ScError) {
        let d = v.discriminant();
        self.write_type_sc_error_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            ScError::Contract(value) => {
                self.write_u32(*value);
            }
            ScError::WasmVm(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Context(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Storage(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Object(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Crypto(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Events(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Budget(value) => {
                self.write_type_sc_error_code(value);
            }
            ScError::Value(value) => {
                self.write_type_sc_error_code(value);
