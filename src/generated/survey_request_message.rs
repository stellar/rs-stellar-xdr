...
impl SurveyRequestMessage {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_survey_request_message(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`SurveyRequestMessage`], mirroring `<SurveyRequestMessage as WriteXdr>::write_xdr`.
    pub const fn write_type_survey_request_message(&mut self, v: &SurveyRequestMessage) {
        self.write_type_node_id(&v.surveyor_peer_id);
        self.write_type_node_id(&v.surveyed_peer_id);
        self.write_u32(v.ledger_num);
        self.write_type_curve25519_public(&v.encryption_key);
