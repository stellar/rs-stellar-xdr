...
impl TrustLineAsset {
    ...
    pub const fn const_xdr_len(&self) -> usize {
        ...
        w.write_type_trust_line_asset(self);
        w.len()
    }
...
#[cfg(feature = "const")]
impl ConstWriter<'_> {
    /// Serializes a [`TrustLineAsset`], mirroring `<TrustLineAsset as WriteXdr>::write_xdr`.
    pub const fn write_type_trust_line_asset(&mut self, v: &TrustLineAsset) {
        let d = v.discriminant();
        self.write_type_asset_type(&d);
        #[allow(clippy::match_same_arms)]
        match v {
            TrustLineAsset::Native => {}
            TrustLineAsset::CreditAlphanum4(value) => {
                self.write_type_alpha_num4(value);
            }
            TrustLineAsset::CreditAlphanum12(value) => {
                self.write_type_alpha_num12(value);
            }
            TrustLineAsset::PoolShare(value) => {
