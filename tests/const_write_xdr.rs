...
//! `View` form; for types that own none they live on the type itself. Output is
//! compared against the streaming `write_xdr` of the owned type (needs `std`).

use std::io::Cursor;
use stellar_xdr::{
    ConstError, ConstWriter, Hash, Limited, Limits, Memo, MemoType, MemoView, ReadXdr, StringMView,
    TimeBounds, TimePoint, TxDemandVector, TxDemandVectorView, Uint256, VecMView, WriteXdr,
};
...
#[test]
fn const_on_view_union_matches_write_xdr() {
    ...
    let cases: [MemoView; 4] = [
        MemoView::None,
        MemoView::Text(StringMView::new_str("hello")),
        MemoView::Id(42),
...
#[test]
fn const_on_view_var_array() {
    // A newtype over `VecM<Hash>`, serialized through its borrowing View form,
    // built from a slice of fixed-size arrays.
    let hashes = [Hash([1u8; 32]), Hash([2u8; 32]), Hash([3u8; 32])];
    let v = TxDemandVectorView(VecMView::new(&hashes));
...
fn const_round_trips() {
    let m = MemoView::Text(StringMView::new_str("round trip"));
    let owned: Memo = (&m).into();
...
// Both an owned non-heap type and a borrowing View type are exercised entirely
// in const contexts.
...
const MEMO: MemoView = MemoView::Text(StringMView::new_str("hi"));
