...
//! It is emitted on the borrowing `Ref` form of a type when the type owns heap
//! data (and therefore has a `Ref` variant), and on the type itself otherwise;
...
/// inside a `Ref` type, where the value is a borrowing `BytesMRef`/`StringMRef`/
/// `VecMRef`; each exposes a const `as_slice`.
///
...
/// `WriteXdr`, which is mirrored here even though the `Ref` form borrows
/// instead of boxing.
fn encode_type(
    type_: &Type,
    type_info: &TypeInfo,
    ...
    match type_ {
        ...
        Type::OpaqueVar(_) | Type::String(_) => {
            format!("w.write_len_prefixed({acc}.as_slice());")
        }
        Type::Ident(_) => {
            ...
            // owned type when it owns no heap, otherwise its `Ref` form).
            if let Some(builtin) = type_info.resolve_typedef_to_builtin(type_) {
                ...
                // `Ref` form borrows instead of boxing but the depth accounting
                // must match.
