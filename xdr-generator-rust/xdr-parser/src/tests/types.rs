...
fn test_compute_properties_kind_values() {
    ...
    "#;
    let props = build_props(input);
    assert!(props.types.contains_key("MAX"));
    assert!(props.types.contains_key("MyInt"));
    assert!(props.types.contains_key("S"));
    assert!(props.types.contains_key("E"));
    assert!(props.types.contains_key("U"));
}
